...
export class DashboardPage implements OnDestroy, OnInit {
  ...
  ionViewDidEnter() {
    /** Prevent going back to login page on swipe left in iOS */
    this.routerOutlet.swipeGesture = false;
  }

  /** Get Auctions for the logged-in buyer */
  getAuction() {
    this.auctionSubscription = this.auctionService
      .getAuctionsForBuyer()
      .subscribe({
        next: (resp: BuyerAuctionsResponse) => {
          if (resp && resp.items) {
  ...
  buildData(item: any): BuyerAuctionDisplayModel {
    return {
      ...
      currentHighestBidValue: item.currentHighestBidValue.toLocaleString(
        "de-DE",
        {
          style: "currency",
          currency: "EUR",
